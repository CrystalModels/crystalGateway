...
Flight::route('POST /changeKeywordSession/@headerslink', function ($headerslink) {
    ...
    if (!empty($headerslink)) {
      ...
      curl_close($curl);
      $url = $sub_domain.'/crystalCore/apiUsers/v1/changeKeywordSession/'.$headerslink;
...
// Crear el array con los valores correspondientes
      ...
      curl_close($curl);
      $url = $sub_domain.'/crystalCore/apiUsers/v1/validateLogInChange/'.$headerslink;
...
Flight::route('POST /sendMessage/@apk/@xapk', function ($apk,$xapk) {
    ...
    if (!empty($apk) && !empty($xapk)) {
      ...
      curl_close($curl);
      $url = $sub_domain."/crystalCore/apiUsers/v1/sendMessage/$response1/$xapk";
    ...
    if (isset($headers['x-api-Key'])) {
      ...
      curl_close($curl);
        ...
        if ($response1 != 'false' ) {
            ...
            // Realizar la solicitud y obtener la respuesta
            $response = file_get_contents($sub_domain.'/crystalCore/apiUsers/v1/getProfileInfoLogChange/'.$userName, false, $context);
